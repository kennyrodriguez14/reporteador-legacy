...
Public Class Efectividad_DIMOSA
    Dim dia As String
    ...
    Sub Visita(ByVal Clave As Integer, ByVal dias As String)
        ...
        If _Sucursal.SelectedItem = "SRC" Then
            dv = EfectividadSRC.VisitadosDIMOSA(Clave, dias)
            If Not IsNothing(EfectividadSRC.VisitadosDIMOSA(Clave, dias)) Then
                _GridDiaVisita.DataSource = dv
            End If
        ElseIf _Sucursal.SelectedItem = "SPS" Then
            dv = EfectividadSPS.VisitadosDIMOSA(Clave, dias)
            If Not IsNothing(EfectividadSPS.VisitadosDIMOSA(Clave, dias)) Then
                _GridDiaVisita.DataSource = dv
            End If
        ElseIf _Sucursal.SelectedItem = "Saba" Then
            dv = EfectividadTocoa.VisitadosDIMOSA(Clave, dias)
            If Not IsNothing(EfectividadTocoa.VisitadosDIMOSA(Clave, dias)) Then
                _GridDiaVisita.DataSource = dv
            End If
    ...
    Sub Num_efectivos(ByVal Clave As Integer)
        ...
        If _Sucursal.SelectedItem = "SRC" Then
            If RadioTodos.Checked = True Then
                DataView = EfectividadSRC.EfectivosDIMOSA(_FechaInicial.Value.Date, Clave)
            Else
                DataView = EfectividadSRC.EfectivosMenores30DIMOSA(_FechaInicial.Value.Date, Clave)
            End If
            If Not IsNothing(EfectividadSRC.EfectivosDIMOSA(_FechaInicial.Value.Date, Clave)) Then
                DGNE.DataSource = DataView
            End If
        ElseIf _Sucursal.SelectedItem = "SPS" Then
            If RadioTodos.Checked = True Then
                DataView = EfectividadSPS.EfectivosDIMOSA(_FechaInicial.Value.Date, Clave)
            Else
                DataView = EfectividadSPS.EfectivosMenores30DIMOSA(_FechaInicial.Value.Date, Clave)
            End If
            If Not IsNothing(EfectividadSPS.EfectivosDIMOSA(_FechaInicial.Value.Date, Clave)) Then
                DGNE.DataSource = DataView
            End If
        ElseIf _Sucursal.SelectedItem = "Saba" Then
            If RadioTodos.Checked = True Then
                DataView = EfectividadTocoa.EfectivosDIMOSA(_FechaInicial.Value.Date, Clave)
            Else
                DataView = EfectividadTocoa.EfectivosMenores30DIMOSA(_FechaInicial.Value.Date, Clave)
            End If
            DataView = EfectividadTocoa.EfectivosDIMOSA(_FechaInicial.Value.Date, Clave)
            If Not IsNothing(EfectividadTocoa.EfectivosDIMOSA(_FechaInicial.Value.Date, Clave)) Then
                DGNE.DataSource = DataView
            End If
    ...
    Sub NumNCDiarias(ByVal Clave As Integer)
        If _Sucursal.SelectedItem = "SRC" Then
            DataView = EfectividadSRC.NotasCDiariasDIMOSA(_FechaInicial.Value.Date, Clave)
            If Not IsNothing(EfectividadSRC.NotasCDiariasDIMOSA(_FechaInicial.Value.Date, Clave)) Then
                _gridNCDiarias.DataSource = DataView
            End If
        ElseIf _Sucursal.SelectedItem = "SPS" Then
            DataView = EfectividadSPS.NotasCDiariasDIMOSA(_FechaInicial.Value.Date, Clave)
            If Not IsNothing(EfectividadSPS.NotasCDiariasDIMOSA(_FechaInicial.Value.Date, Clave)) Then
                _gridNCDiarias.DataSource = DataView
            End If
        ElseIf _Sucursal.SelectedItem = "Saba" Then
            DataView = EfectividadTocoa.NotasCDiariasDIMOSA(_FechaInicial.Value.Date, Clave)
            If Not IsNothing(EfectividadTocoa.NotasCDiariasDIMOSA(_FechaInicial.Value.Date, Clave)) Then
                _gridNCDiarias.DataSource = DataView
            End If
    ...
    Sub Cargar()
        Try
            If _Sucursal.SelectedItem = "SPS" Then
                DataView = _SPS.DatosDIMOSA()
                If Not IsNothing(_SPS.DatosDIMOSA()) Then
                    ListaVendedores.DataSource = DataView
                End If
            ElseIf _Sucursal.SelectedItem = "SRC" Then
                DataView = _SRC.DatosDIMOSA()
                If Not IsNothing(_SRC.DatosDIMOSA()) Then
                    ListaVendedores.DataSource = DataView
                End If
            ElseIf _Sucursal.SelectedItem = "Saba" Then
                DataView = _Tocoa.DatosDIMOSA()
                If Not IsNothing(_Tocoa.DatosDIMOSA()) Then
                    ListaVendedores.DataSource = DataView
                End If
    ...
    Private Sub EnviarACorreoToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If activo = 1 Then
            CorreoAdjunto()
            Correo.DocAdjunto = "C:\Reportes\Efectividad\Reporte de Efectividad DIMOSA.XLSX"
    ...
    Sub CorreoAdjunto()
        Try
            ...
            Next
            wSheet.Columns.AutoFit()
            wBook.SaveAs("C:\Reportes\Efectividad\Reporte de Efectividad DIMOSA.XLSX")
            excel.Quit()
