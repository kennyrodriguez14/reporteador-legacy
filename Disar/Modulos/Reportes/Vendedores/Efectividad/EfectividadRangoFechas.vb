Imports Disar.data
Public Class EfectividadRangoFechas
            ...
            Else
                ...
                For c As Integer = 0 To _GridVendedores.RowCount - 1
                    If _GridVendedores.Rows(c).Cells(2).Value = "DET" Then
                        ...
                        dt = db.ClientesIdeales(_cmbFechai.Value.Year, _cmbFechai.Value.Month, _GridVendedores.Rows(c).Cells(0).Value)
                        ...
                        Dim contador As Integer = _cmbFechai.Value.Day
                        Dim Fecha As Date = _cmbFechai.Value
                        Dim Cantidad_Dias = DateDiff(DateInterval.Day, _cmbFechai.Value.Date, _cmbFechaf.Value.Date)
                        Cantidad_Dias += 1
                        Dim domingos As Integer = 0

                        Fecha = _cmbFechai.Value
                        For j As Integer = contador To Cantidad_Dias
                        ...
                        'Dim TextDiasMensuales As Integer = Cantidad_Dias - domingos
                        Dim TextDiasTrabajados As Integer = Cantidad_Dias - domingos

                        Lineas("Clientes Visitados") = ((TotalSemana / 6) * TextDiasTrabajados)
                        Lineas("Efectivos") = TotalEfectivos
                        If TotalVisitas = 0 Then
                            Lineas("Porcentaje") = 0 & " %"
                        Else
                            Lineas("Porcentaje") = Math.Round(TotalEfectivos / ((TotalSemana / 6) * TextDiasTrabajados) * 100) & " %"
                        End If
    ...
    Sub ListaEmpleados()
        Try
            DataView = Clase_ListaVendedores.datos()
            If Not IsNothing(Clase_ListaVendedores.datos()) Then
    ...
    Sub Visita(ByVal Clave As Integer, ByVal dias As String, ByVal dias1 As String, ByVal dias2 As String)
        dv = Clase_Efectivos.VisitadosRangoFechas(Clave, dias, dias1, dias2)
        If Not IsNothing(Clase_Efectivos.VisitadosRangoFechas(Clave, dias, dias1, dias2)) Then
    ...
    Sub Efectivos(ByVal Fecha As Date, ByVal Fecha2 As Date, ByVal Clave As Integer)
        Try
            If RadioTodos.Checked = True Then
                DataView = Clase_Efectivos.EfectivosRangoFechas(Fecha, Fecha2, Clave)
            Else
                DataView = Clase_Efectivos.EfectivosRangoFechasMenores30(Fecha, Fecha2, Clave)
            End If

            If Not IsNothing(Clase_Efectivos.EfectivosRangoFechas(Fecha, Fecha2, Clave)) Then
    ...
    Sub NotasCredto(ByVal Fecha As Date, ByVal Fecha2 As Date, ByVal Clave As Integer)
        'Try
        DataView = Clase_Efectivos.NotasCredito(Fecha, Fecha2, Clave)
        If Not IsNothing(Clase_Efectivos.NotasCredito(Fecha, Fecha2, Clave)) Then
    ...
    Private Sub ModificarToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ModificarToolStripMenuItem.Click
        FormModificarIdeales.ShowDialog()
    End Sub
