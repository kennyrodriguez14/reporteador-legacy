...
Public Class frm_guia_remision_papeleria
    ...
    Sub cargar_remitente()
        Try
            cmb_remite.DataSource = remisiones.Remite_BIENESYRECURSOS(_Inicio.lblId.Text)
            cmb_remite.DisplayMember = "DESCRIPCION"
            cmb_remite.ValueMember = "SUCURSAL_ID"
            Dim dt As New DataTable
            dt = remisiones.DatosSucursal_BIENESYRECURSOS(cmb_remite.SelectedValue)
    ...
    Sub cargar_entradas()
        Try
            If lbl_empresa.Text = "ADMON BIENES Y RECURSOS" Then
                cmb_concepto_entrada.DataSource = remisiones.conceptos_entradas_BIENESRECURSOS()
    ...
    Sub cargar_salidas()
        Try
            If lbl_empresa.Text = "ADMON BIENES Y RECURSOS" Then
                cmb_concepto_salida.DataSource = remisiones.conceptos_salidas_BIENESYRECURSOS()
    ...
    Sub cargar_destino(ByVal SUCURSAL As Integer, ByVal EMPRESA As String)
        Try
            cmb_destino.DataSource = remisiones.Destino_BIENESYRECURSOS(SUCURSAL, EMPRESA)
            cmb_destino.DisplayMember = "DESCRIPCION"
            cmb_destino.ValueMember = "SUCURSAL_ID"
            Dim dt As New DataTable
            dt = remisiones.DatosSucursal_BIENESYRECURSOS(cmb_destino.SelectedValue)
            ...
            If validador2 = 0 Then
                ...
                If validador = 0 Then
                    ...
                    If cmb_otro_destino.Checked = True Then
                        ...
                        If lbl_empresa.Text = "ADMON BIENES Y RECURSOS" Then
                            fallo = remisiones.INSERTAR_DESTINO_EXTERNO_BIENESYRECURSOS(dt, dt2, txt_destino.Text, txt_rtn.Text, txt_direccion.Text, _
                                    otro_transportista, otro_vehiculo, _txt_otro_transportista.Text, txt_rtn_otro_transportista.Text, _
                                    txt_marca_vehiculo.Text, txt_nplaca_vehiculo.Text)
                        End If
                    ...
                    Else
                        ...
                        If lbl_empresa.Text = "ADMON BIENES Y RECURSOS" Then
                            fallo = remisiones.NUEVASOLICITUDBIENESYRECURSOS(dt, dt2, otro_transportista, otro_vehiculo, _txt_otro_transportista.Text, _
                                                    txt_rtn_otro_transportista.Text, txt_marca_vehiculo.Text, txt_nplaca_vehiculo.Text, _Inicio.lblUsuario.Text)
                        End If
                    End If
    ...
    Sub datos_destino()
        ...
        Else
            ...
            tabla_destino = remisiones.DatosEmpresa_BIENESYRECURSOS(cmb_destino.SelectedValue)
            ...
            If dgv_detalles.Rows(i).Cells(2).Value <> "" Then
                ...
                If lbl_empresa.Text = "ADMON BIENES Y RECURSOS" Then
                    If dgv_detalles.Rows(i).Cells(6).Value = "S" Then
                        existencia = productos.ValidarExistencia_BIENESYRECURSOS(Val(lbl_almacen.Text), dgv_detalles.Rows(i).Cells(2).Value)
                    Else
                        existencia = productos.ValidarExistencia_BIENESYRECURSOS_inventario(Val(lbl_almacen.Text), dgv_detalles.Rows(i).Cells(2).Value)
                    End If
    ...
    Sub validar_si_existe()
        ...
        For i As Integer = 0 To dgv_detalles.RowCount - 1
            If dgv_detalles.Rows(i).Cells(2).Value <> "" Then
                If lbl_empresa.Text = "ADMON BIENES Y RECURSOS" Then
                    If productos.ValidarSIexiste_prod_BIENESYRECURSOS(Val(lbl_almacen_destino.Text), dgv_detalles.Rows(i).Cells(2).Value) = True Then
                    Else
                        validador2 += 1
