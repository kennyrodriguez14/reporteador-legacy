...
Public Class frm_recurrencias_formato_nuevo_factura
    ...
    Sub guardar()
        ...
        For index As Integer = 0 To grdFactura.Rows.Count - 1
            If grdFactura.Rows(index).Cells(1).Value <> "" Then
                conexion.Insert(cmbFecha.Value.Date, grdFactura.Rows(index).Cells(1).Value, grdFactura.Rows(index).Cells(4).Value _
                                , grdFactura.Rows(index).Cells(7).Value, grdFactura.Rows(index).Cells(9).Value, grdFactura.Rows(index).Cells(11).Value, cmb_almacen.SelectedValue, cmb_empresa.Text)
            End If
        Next
