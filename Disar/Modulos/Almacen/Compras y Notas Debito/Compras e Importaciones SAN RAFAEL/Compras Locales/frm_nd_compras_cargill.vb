Imports Disar.data
Public Class frm_nd_compras_cargill
    ...
    Private Sub btn_new_prov_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_new_prov.Click
        ...
        frm_lista_proveedores.cargill = "S"
    ...
    Sub cargar_producto_rutina()
        ...
        If frm_productos_precios_neg.ShowDialog = Windows.Forms.DialogResult.OK Then
            ...
            txt_precio_neg.Text = Math.Round(frm_productos_precios_neg.precio_neg, 6)
            txt_descuento_por.Text = Val(txt_descuento_general.Text)
            txt_uni_entrada.Text = frm_productos_precios_neg.uni_entrada
            txt_peso.Text = frm_productos_precios_neg.peso
        ...
        Try
            ...
            Else
                ...
                If cmb_uni.SelectedItem = "cj" Then
                    ...
                    descuento_mostrador = ((Val(txt_factor_conversion.Text) * Val(txt_precio_final.Text) * Val(txt_cantidad.Text)) / _
                                           (1 - Val(txt_descuento_por.Text) / 100)) - ((Val(txt_factor_conversion.Text) * _
                                                                                        Val(txt_precio_final.Text) * Val(txt_cantidad.Text)))
                    isv_mostrador = ((Val(txt_factor_conversion.Text) * Val(txt_precio_final.Text) * Val(txt_cantidad.Text)) * _
                                     (Val(txt_isv_por.Text) / 100))
                    subtotal_mostrador = ((Val(txt_factor_conversion.Text) * Val(txt_precio_final.Text) * Val(txt_cantidad.Text)) / _
                                          (1 - Val(txt_descuento_por.Text) / 100))
                    total_mostrador = (Val(txt_factor_conversion.Text) * Val(txt_precio_final.Text) * Val(txt_cantidad.Text)) + _
                    ((Val(txt_factor_conversion.Text) * Val(txt_precio_final.Text) * Val(txt_cantidad.Text)) * (Val(txt_isv_por.Text) / 100))

                ElseIf cmb_uni.SelectedItem = "pz" Then
                    subtotal = precio_multiplicador * Val(txt_cantidad.Text)

                    descuento_mostrador = ((Val(txt_precio_final.Text) * Val(txt_cantidad.Text)) / (1 - Val(txt_descuento_por.Text) / 100)) - _
                    (Val(txt_precio_final.Text) * Val(txt_cantidad.Text))
                    ...
                    total_mostrador = (Val(txt_precio_final.Text) * Val(txt_cantidad.Text)) + ((Val(txt_precio_final.Text) * _
                                                                                Val(txt_cantidad.Text)) * (Val(txt_isv_por.Text) / 100))
                Else
                    subtotal = Val(txt_factor_conversion.Text) * precio_multiplicador * Val(txt_cantidad.Text)

                    descuento_mostrador = ((Val(txt_factor_conversion.Text) * Val(txt_precio_final.Text) * Val(txt_cantidad.Text)) / _
(1 - Val(txt_descuento_por.Text) / 100)) - ((Val(txt_factor_conversion.Text) * Val(txt_precio_final.Text) * Val(txt_cantidad.Text)))
                ...
                grd_ingreso.Rows.Add(Val(txt_cantidad.Text), txt_codigo_producto.Text, txt_nombre_producto.Text, txt_factor_conversion.Text, _
                                     ...
                                     Math.Round(descuento_mostrador), Math.Round(isv_mostrador), 0, Val(txt_peso.Text), txt_con_lote.Text)
                limpiar()
                sumar_totales()
                txt_cantidad.Focus()
    ...
    Sub sumar_totales()
        ...
        total_mostrador = total_mostrador - desc_pp2

        total_mostrador += Val(txt_isv_flete.Text)
        subtotal += Val(txt_flete.Text)
        ...
        Dim st As Double = 0
        st = subtotal - Val(txt_descuentopp1.Text.Replace(",", ""))
        ...
        Try
            ...
            Else
                ...
                dt_e.Rows.Add(txt_codigo_proveedor.Text, txt_nombre_proveedor.Text, cmb_sucursal.SelectedValue, txt_referencia_proveedor.Text, _
                    ...
                    , Convert.ToDouble((txt_isvmostrador.Text)), Convert.ToDouble((txt_totalmostrador.Text)), Convert.ToDouble(Val(txt_flete.Text)), _
                    txtNumFlete.Text, cmb_fecha_documento.Value, Convert.ToDouble(Val(txt_isv_flete.Text)), Convert.ToDouble(Val(txt_total_flete.Text)))
                ...
                For index As Integer = 0 To grd_ingreso.Rows.Count - 1
                    dt_p.Rows.Add(grd_ingreso.Rows(index).Cells(4).Value, grd_ingreso.Rows(index).Cells(1).Value, grd_ingreso.Rows(index).Cells(2).Value, _
                        ...
                        , grd_ingreso.Rows(index).Cells(15).Value, grd_ingreso.Rows(index).Cells(16).Value, grd_ingreso.Rows(index).Cells(18).Value _
                                  , grd_ingreso.Rows(index).Cells(19).Value, grd_ingreso.Rows(index).Cells(17).Value, _
                                  grd_ingreso.Rows(index).Cells(20).Value, grd_ingreso.Rows(index).Cells(21).Value, _
                                  grd_ingreso.Rows(index).Cells(22).Value)
                Next
                ...
                ElseIf modo = "modificar" Then
                    ...
                    If resultado = "correcto" Then
                        If MessageBox.Show("¿Desea enviar la compra a SAE?", "Confirmacion", MessageBoxButtons.YesNoCancel, _
                                           MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                            resultado = conexion.envio_individual_compras_SAE_CARGILL(dt_e, dt_p, codigo_compra, conlote, lote)
                        End If
                    End If
                End If
                If resultado = "correcto" Then
        ...
        Try
            ...
            For index As Integer = 0 To dt.Rows.Count - 1
                grd_ingreso.Rows.Add(dt.Rows(index)(4), dt.Rows(index)(1), dt.Rows(index)(2), dt.Rows(index)(3), _
                                     ...
                                     dt.Rows(index)(18), dt.Rows(index)(16), dt.Rows(index)(17), dt.Rows(index)(19), _
                                     dt.Rows(index)(20), dt.Rows(index)(21))
            Next
    ...
    Private Sub cmb_fecha_documento_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmb_fecha_documento.ValueChanged, cmb_factura.ValueChanged
    ...
    End Sub
    'fecah doc
    Private Sub cmb_fecha_documento_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cmb_fecha_documento.KeyPress, cmb_factura.KeyPress
        If Asc(e.KeyChar) = Keys.Enter Then
    ...
    Private Sub txt_descuento_general_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txt_descuento_general.KeyPress, txt_descuento_pp.KeyPress
        If Asc(e.KeyChar) = Keys.Enter Then
    ...
    Private Sub frm_nd_compras_cargill_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub txt_descuento_general_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txt_descuento_general.TextChanged
        ...
        For index As Integer = 0 To grd_ingreso.Rows.Count - 1
            grd_ingreso.Rows(index).Cells(7).Value = Math.Round(Val(txt_descuento_por.Text), 6)
        Next
