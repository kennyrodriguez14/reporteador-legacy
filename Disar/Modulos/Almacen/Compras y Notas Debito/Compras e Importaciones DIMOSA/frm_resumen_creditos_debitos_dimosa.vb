Imports Disar.data
Public Class frm_resumen_debitos_creditos_dimosa

    Dim Conexion As New cls_notas_debito

    Private Sub frm_resumen_debitos_creditos_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        cargar_proveedores()
    End Sub

    Sub cargar_proveedores()
        Try
            cmbProveedor.DataSource = Conexion.resumen_debitosycreditos_DIMOSA(Now.Date, Now.Date, "", "LIS")
    ...
    Private Sub _btnGenerar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles _btnGenerar.Click
        Try
            grd_reporte.DataSource = Conexion.resumen_debitosycreditos_DIMOSA(_FechaInicial.Value.Date, _fechafinal.Value.Date, cmbProveedor.SelectedValue, "REP")
        Catch ex As Exception
            MessageBox.Show(ex.Message)
        End Try
    End Sub
