...
Public Class frm_nueva_compra_nacional_opl
    ...
    Private Sub btn_new_prov_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_new_prov.Click
        frm_lista_proveedores_opl.Close()
        If frm_lista_proveedores_opl.ShowDialog = Windows.Forms.DialogResult.OK Then
            txt_codigo_proveedor.Text = frm_lista_proveedores_opl.codigo
            txt_nombre_proveedor.Text = frm_lista_proveedores_opl.nombre
            txt_rfc.Text = frm_lista_proveedores_opl.rfc
            cmb_sucursal.Focus()
        End If
    End Sub

    Private Sub frm_nueva_compra_nacional_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        TextPedido.Enabled = True
    ...
    Sub cargar_producto_rutina()
        frm_productos_precios_neg_opl.Close()
        frm_productos_precios_neg_opl.proveedor = txt_codigo_proveedor.Text
        frm_productos_precios_neg_opl.almacen = cmb_sucursal.SelectedValue
        If frm_productos_precios_neg_opl.ShowDialog = Windows.Forms.DialogResult.OK Then
            txt_codigo_producto.Text = frm_productos_precios_neg_opl.codigo
            txt_nombre_producto.Text = frm_productos_precios_neg_opl.producto
            txt_factor_conversion.Text = frm_productos_precios_neg_opl.fac_conv
            txt_isv_por.Text = frm_productos_precios_neg_opl.isv_por
            txt_precio_neg.Text = Math.Round(frm_productos_precios_neg_opl.precio_neg, 6)
            txt_descuento_por.Text = Val(txt_descuento_general.Text)
            txt_uni_entrada.Text = frm_productos_precios_neg_opl.uni_entrada
            txt_con_lote.Text = frm_productos_precios_neg_opl.con_lote
    ...
    Private Sub btn_agregar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_agregar.Click
        Try
            ...
            Else
                ...
                grd_ingreso.Rows.Add(Val(txt_cantidad.Text), txt_codigo_producto.Text, txt_nombre_producto.Text, txt_factor_conversion.Text,
                                     cmb_uni.SelectedItem, Math.Round(Val(txt_precio_neg.Text), 6), Math.Round(Val(txt_precio_final.Text), 6),
                                     Math.Round(Val(txt_descuento_por.Text), 6), Val(txt_isv_por.Text), Math.Round(subtotal, 6),
                                     Math.Round(descuento, 6), Math.Round(isv, 6), Math.Round(total, 6), Math.Round(precio_multiplicador, 6),
                                     cadena_ajuste, txt_uni_entrada.Text, Math.Round(subtotal_mostrador, 6), Math.Round(total_mostrador, 6),
        ...
        Try
            ...
            Else
                ...
                Else
                    ...
                    dt_e.Rows.Add(txt_codigo_proveedor.Text, txt_nombre_proveedor.Text, cmb_sucursal.SelectedValue, txt_referencia_proveedor.Text,
                                  txt_rfc.Text, cmb_fecha_documento.Value.Date, txt_descuento_general.Text, Convert.ToDouble(txt_subtotal_final.Text),
                                  Convert.ToDouble(txt_descuento_final.Text), Convert.ToDouble(txt_isv_final.Text), Convert.ToDouble(txt_total_final.Text),
                    ...
                    For index As Integer = 0 To grd_ingreso.Rows.Count - 1
                        dt_p.Rows.Add(grd_ingreso.Rows(index).Cells(4).Value, grd_ingreso.Rows(index).Cells(1).Value, grd_ingreso.Rows(index).Cells(2).Value,
                                      grd_ingreso.Rows(index).Cells(3).Value, grd_ingreso.Rows(index).Cells(0).Value, grd_ingreso.Rows(index).Cells(5).Value,
                                      grd_ingreso.Rows(index).Cells(6).Value, grd_ingreso.Rows(index).Cells(7).Value, grd_ingreso.Rows(index).Cells(8).Value,
                                      grd_ingreso.Rows(index).Cells(9).Value, grd_ingreso.Rows(index).Cells(10).Value, grd_ingreso.Rows(index).Cells(11).Value,
                    ...
                    ValidaEnvio()
                    If modo = "nuevo" Then
                        If MessageBox.Show("¿Desea enviar la compra a SAE?", "Confirmacion", MessageBoxButtons.YesNoCancel,
                                           MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                            resultado = conexion.guardar_compras_local_envio_SAE_opl(dt_e, dt_p, conlote, lote, cmb_fecha_vencimiento.Value, _Inicio.lblUsuario.Text, TextPedido.Text)
                        Else
                            resultado = conexion.guardar_compras_local_OPL(dt_e, dt_p, lote, cmb_fecha_vencimiento.Value, TextPedido.Text)
                        End If

                    ElseIf modo = "modificar" Then
                        resultado = conexion.modificar_compras_local_opl(dt_e, dt_p, codigo_compra, lote, cmb_fecha_vencimiento.Value)
                        If resultado = "correcto" Then
                            If MessageBox.Show("¿Desea enviar la compra a SAE?", "Confirmacion", MessageBoxButtons.YesNoCancel,
                                               MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                                resultado = conexion.envio_individual_compras_SAE_OPL(dt_e, dt_p, codigo_compra, conlote, lote, cmb_fecha_vencimiento.Value, _Inicio.lblUsuario.Text)
                            End If
                        End If
                    End If

                    If resultado = "correcto" Then
                        MessageBox.Show("Informacion Almacenada correctamente", "Operacion Exitosa", MessageBoxButtons.OK,
        ...
        If Val(txt_cantidad.Text) > 0 Then
            ...
            If frm_calculadora_totales.ShowDialog() = Windows.Forms.DialogResult.OK Then
                ...
                If cmb_valores_base.SelectedItem = "Sub Total" Then
                    ...
                    If cmb_uni.SelectedItem = "cj" Then
                        precio = Math.Round(Val(frm_calculadora_totales.txt_totales.Text) / Val(txt_cantidad.Text) / Val(txt_factor_conversion.Text), 6)
                    ElseIf cmb_uni.SelectedItem = "pz" Then
                        precio = Math.Round(Val(frm_calculadora_totales.txt_totales.Text) / Val(txt_cantidad.Text), 6)
                    Else
                        precio = Math.Round(Val(frm_calculadora_totales.txt_totales.Text) / Val(txt_cantidad.Text) / Val(txt_factor_conversion.Text), 6)
                    End If
    ...
    Sub cargar_partidas()
        Try
            ...
            dt = conexion.listar_partidas_OPL(codigo_compra)
            For index As Integer = 0 To dt.Rows.Count - 1
                grd_ingreso.Rows.Add(dt.Rows(index)(4), dt.Rows(index)(1), dt.Rows(index)(2), dt.Rows(index)(3),
                                     dt.Rows(index)(0), dt.Rows(index)(5), dt.Rows(index)(6), dt.Rows(index)(7),
                                     dt.Rows(index)(8), dt.Rows(index)(9), dt.Rows(index)(10), dt.Rows(index)(11),
                                     dt.Rows(index)(12), dt.Rows(index)(13), dt.Rows(index)(14), "cj", dt.Rows(index)(15),
    ...
    Private Sub VerNotas_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles VerNotas.Click
        ...
        FrmNotasCreDeb.DataDebito1.Rows.Clear()
        FrmNotasCreDeb.DataCredito.Rows.Clear()
        For i As Integer = 0 To grd_ingreso.RowCount - 1
            ...
            Try
                ...
                dta = db.listar_productos_opl(grd_ingreso.Rows(i).Cells(1).Value, txt_codigo_proveedor.Text)
                PrecioNeg = dta(0)(3)
            Catch ex As Exception
            ...
            If (grd_ingreso.Rows(i).Cells("precio_final").Value < grd_ingreso.Rows(i).Cells("Precio_Negociado").Value) Then
                ...
                FrmNotasCreDeb.DataCredito.Rows.Add(grd_ingreso.Rows(i).Cells("cve_art").Value, grd_ingreso.Rows(i).Cells("producto").Value, ValorNota)
            End If

            If (grd_ingreso.Rows(i).Cells("precio_final").Value > grd_ingreso.Rows(i).Cells("Precio_Negociado").Value) Then
                ValorNota = Math.Round((grd_ingreso.Rows(i).Cells("precio_negociado").Value * grd_ingreso.Rows(i).Cells("fconversion").Value * grd_ingreso.Rows(i).Cells("cantidad").Value) - (grd_ingreso.Rows(i).Cells("precio_final").Value * grd_ingreso.Rows(i).Cells("fconversion").Value * grd_ingreso.Rows(i).Cells("cantidad").Value), 2)
                FrmNotasCreDeb.DataDebito1.Rows.Add(grd_ingreso.Rows(i).Cells("cve_art").Value, grd_ingreso.Rows(i).Cells("producto").Value, ValorNota)
            End If
        Next
        FrmNotasCreDeb.ShowDialog()
    ...
    Sub CargaCompleta()
        ...
        dtx = db.CargaInfoPedido(Val(TextPedido.Text), "SAN RAFAEL")
        If dtx.Rows.Count > 0 Then
            ...
            proveedor = dtx(0)(2)
            'FLAG
            FormCargaTodasPartidasCompraNacional.DataDatos.Rows.Clear()
            FormCargaTodasPartidasCompraNacional.Pedido = Val(TextPedido.Text)
            For a As Integer = 0 To dtx.Rows.Count - 1
                Dim dt As New DataTable
                dt = Conexion.valida_productos(dtx(a)(5), dtx(a)(4))
                Dim validar As Integer = 0
                validar = dt.Rows(0)(0)
                Dim grd_lista As New DataTable
                grd_lista = Conexion.listar_productos(dtx(a)(5), dtx(0)(2))
                ...
                Try
                    ...
                    Else
                        ...
                        FormCargaTodasPartidasCompraNacional.DataDatos.Rows.Add(Val(dtx(a)(7)) - Val(dtx(a)(8)), "cj", codigo, producto, fac_conv, uni_entrada, "...", "", precio_neg, Val(txt_descuento_general.Text), isv_por, con_lote, CODPROV, "✔", Val(dtx(a)(7)) - Val(dtx(a)(8)))
                        btn_new_prov.Enabled = False
                ...
                End Try
            Next
            FormCargaTodasPartidasCompraNacional.ShowDialog()
        Else
    ...
    '    Dim Conexion As New cls_notas_debito
    '    Dim proveedor As String = ""
    '    dtx = db.CargaInfoPedido(Val(TextPedido.Text))
    '    If dtx.Rows.Count > 0 Then
    '        txt_codigo_proveedor.Text = dtx(0)(2)
    '        txt_nombre_proveedor.Text = dtx(0)(3)
    '        cmb_sucursal.SelectedValue = dtx(0)(4)
    ...
    '            If validar < 1 Then
    '                MessageBox.Show("El producto Seleccionado no esta asignado al Almacen " & dtx(0)(4), "Producto no Asignado", MessageBoxButtons.OK, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button1)
    '  

    Private Sub BtnDevolver_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDevolver.Click
        Dim db As New cls_notas_debito
        Try
            FrmDevolucionCompraNacional.txt_codigo_proveedor.Text = txt_codigo_proveedor.Text
            FrmDevolucionCompraNacional.txt_nombre_proveedor.Text = txt_nombre_proveedor.Text
            FrmDevolucionCompraNacional.txt_almacen.Text = cmb_sucursal.SelectedValue
            FrmDevolucionCompraNacional.txt_referencia_proveedor.Text = txt_referencia_proveedor.Text
            FrmDevolucionCompraNacional.txt_rfc.Text = txt_rfc.Text
            FrmDevolucionCompraNacional.txt_lote.Text = txt_lote.Text
            FrmDevolucionCompraNacional.cmb_fecha_documento.Value = cmb_fecha_documento.Value
            FrmDevolucionCompraNacional.cmb_fecha_factura.Value = cmb_fecha_factura.Value
            FrmDevolucionCompraNacional.txt_descuento_general.Text = txt_descuento_general.Text
            FrmDevolucionCompraNacional.cmb_lote.CheckState = cmb_lote.CheckState
            FrmDevolucionCompraNacional.cmb_fecha_vencimiento.Value = cmb_fecha_vencimiento.Value

            FrmDevolucionCompraNacional.Grd_Partidas.DataSource = Nothing
            FrmDevolucionCompraNacional.Grd_Partidas.Columns.Clear()


            FrmDevolucionCompraNacional.Grd_Partidas.DataSource = db.listar_partidas_compras(codigo_compra)
            For Each column As DataGridViewColumn In FrmDevolucionCompraNacional.Grd_Partidas.Columns
                column.ReadOnly = True
            Next
            Dim ColumnaDevolucion As New DataGridViewTextBoxColumn

            ColumnaDevolucion.Name = "Devolucion"
            ColumnaDevolucion.HeaderText = "Cantidad A Devolver"

            FrmDevolucionCompraNacional.Grd_Partidas.Columns.Add(ColumnaDevolucion)
            FrmDevolucionCompraNacional.cmb_valores_base.SelectedValue = cmb_valores_base.SelectedValue

            FrmDevolucionCompraNacional.txt_subtotal_final.Text = txt_subtotal_final.Text
            FrmDevolucionCompraNacional.txt_descuento_final.Text = txt_descuento_final.Text
            FrmDevolucionCompraNacional.txt_isv_final.Text = txt_isv_final.Text
            FrmDevolucionCompraNacional.txt_total_final.Text = txt_total_final.Text
            FrmDevolucionCompraNacional.Compra = codigo_compra
            FrmDevolucionCompraNacional.CompraSAE = CompraSAE
            For Each row As DataGridViewRow In FrmDevolucionCompraNacional.Grd_Partidas.Rows
                row.Cells("Devolucion").Value = row.Cells(4).Value
            Next
            FrmDevolucionCompraNacional.ComboTipo.SelectedIndex = 0
            If CompraSAE <> "" Then
                FrmDevolucionCompraNacional.ShowDialog()
            End If
    ...
    Sub ValidaEnvio()
        ...
        Dim ASD As String = "<h4>Los siguientes productos presentan diferencias de precios en compra OPL a " & txt_nombre_proveedor.Text & ":</h4></hr></hr>"
        ...
        For Each row As DataGridViewRow In grd_ingreso.Rows
            If row.Cells("precio_negociado").Value <> row.Cells("precio_final").Value Then
                Contador += 1
                ASD += "<tr>"
                ASD += "<td><h5>" & row.Cells(1).Value & " </h5></td> " &
                       "<td><h5>" & row.Cells(2).Value & " </h5></td> " &
                       "<td><h5>" & row.Cells("precio_negociado").Value & " </h5></td> " &
                       "<td><h5>" & row.Cells("precio_final").Value & " </h5> </td>" &
        ...
        If Contador > 0 Then
            ...
            Try
                ...
                correo.To.Add("ricardo.hernandez@sanrafaelhn.com")
    ...
    Dim _excelApp As Microsoft.Office.Interop.Excel.Application
    Dim work As Microsoft.Office.Interop.Excel.Workbook
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        OpenFD.Title = "Seleccionar Plantilla de Carga"
        ...
        Try
            If OpenFD.ShowDialog = DialogResult.OK Then
                x = OpenFD.FileName
                Dim _excelApp As New Microsoft.Office.Interop.Excel.Application()
                work = _excelApp.Workbooks.Open(x)

                Dim Hoja As Integer = 1
                Dim sheet As Microsoft.Office.Interop.Excel.Worksheet = CType(work.Sheets(Hoja), Microsoft.Office.Interop.Excel.Worksheet)
                ...
                dt.Columns.Add("UnidadEnt")
                dt.Columns.Add("Pt1")
                dt.Columns.Add("PrecioFinal")
                dt.Columns.Add("PrecioNego")
                dt.Columns.Add("Descuento")
                dt.Columns.Add("ISV")
                dt.Columns.Add("CodProv")
                dt.Columns.Add("Acept")

                Dim SSS As String = ""

                Try
                    For idx As Integer = 6 To 20
                        Try
                            Dim valueCod As String = (CType(sheet.Cells(idx, 2), Microsoft.Office.Interop.Excel.Range)).Value2
                            Dim InsCantidad As Integer = (CType(sheet.Cells(idx, 7), Microsoft.Office.Interop.Excel.Range)).Value2
                            Dim CodProd As String = ""
                            Dim DTprod As New DataTable()
                            Try
                                DTprod = conexion.listar_productos_opl_codProv2(valueCod, "")
                                If DTprod.Rows.Count > 0 Then
                                Else
                                    DTprod = conexion.listar_productos_opl_codProv(valueCod, "")
                                End If
                            Catch ex As Exception
                            End Try
                            'Try
                            ...
                            Dim InsCodigo As String = DTprod(0)(0)
                            'Try
                            '    SSS = SSS & InsCodigo & vbNewLine
                            'Catch ex As Exception
                            'End Try
                            Dim InsProducto As String = DTprod(0)(1)
                            Dim InsFactor As Integer = DTprod(0)(2)
                            Dim InsUnidad As String = "pz"
                            Dim InsUniEnt As String = DTprod(0)(5)
                            Dim InsPrecio As Decimal = (CType(sheet.Cells(idx, 6), Microsoft.Office.Interop.Excel.Range)).Value2
                            Dim InsImpuesto As Decimal = DTprod(0)(4)
                            Dim InsDescuento As Decimal = Val(txt_descuento_general.Text)
                            Dim InsPeso As Decimal = DTprod(0)(6)
                            Dim InsConLote As String = DTprod(0)(7)
                            FrmCargaCompraOPL.DataDatos.Rows.Add(InsCantidad, "pz", InsCodigo, InsProducto, InsFactor, InsUniEnt, "...", InsPrecio, InsPrecio, InsDescuento, InsImpuesto, InsConLote, valueCod, "✔")
                        Catch ex As Exception
                        End Try
                    Next

                Catch ex As Exception
                    MsgBox("Error en carga general" & ex.Message)
                End Try
                'MsgBox(SSS)
                btn_new_prov.Enabled = False
                FrmCargaCompraOPL.ShowDialog()
