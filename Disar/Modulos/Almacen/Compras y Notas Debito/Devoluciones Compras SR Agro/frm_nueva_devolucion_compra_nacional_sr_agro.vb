Imports Disar.data
Public Class frm_nueva_devolucion_compra_nacional_sr_agro
    ...
    Sub cargar_partidas_devo()
        ...
        grd_ingreso.DataSource = conexion.lista_partidas_devolucion_compra_nacional(cve_compra)
    End Sub
    ...
    Sub partidas()
        Try
            grd_ingreso.DataSource = conexion.lista_partidas_devolver(txt_cve_compra.Text)
    ...
    Sub lista_compras()
        Try
            frm_lista_compras_dev_sr_agro.Close()
            frm_lista_compras_dev_sr_agro.tipo_compra = "cn"
            If frm_lista_compras_dev_sr_agro.ShowDialog() = Windows.Forms.DialogResult.OK Then
                txt_cve_compra.Text = frm_lista_compras_dev_sr_agro.cve_compra
                txt_codigo_proveedor.Text = frm_lista_compras_dev_sr_agro.cve_prov
                txt_nombre_proveedor.Text = frm_lista_compras_dev_sr_agro.prov
                txt_referencia_proveedor.Text = frm_lista_compras_dev_sr_agro.fact
                cmb_sucursal.SelectedValue = frm_lista_compras_dev_sr_agro.almacen
                txt_rfc.Text = frm_lista_compras_dev_sr_agro.rfc
                cmb_fecha_documento.Value = frm_lista_compras_dev_sr_agro.fecha
                txt_descuento_general.Text = frm_lista_compras_dev_sr_agro.desc_por
                txt_subtotal_final.Text = frm_lista_compras_dev_sr_agro.subtotal
                txt_descuento_final.Text = frm_lista_compras_dev_sr_agro.desc_val
                txt_isv_final.Text = frm_lista_compras_dev_sr_agro.isv_val
                txt_total_final.Text = frm_lista_compras_dev_sr_agro.total
                txtn_fact_sae.Text = frm_lista_compras_dev_sr_agro.n_fact_sae
                txt_subtotalmostrador.Text = frm_lista_compras_dev_sr_agro.sub_most
                txt_descuento_mostrador.Text = frm_lista_compras_dev_sr_agro.des_most
                txt_isvmostrador.Text = frm_lista_compras_dev_sr_agro.isv_most
                txt_totalmostrador.Text = frm_lista_compras_dev_sr_agro.total_most
                txt_flete.Text = frm_lista_compras_dev_sr_agro.flete
                txt_lote.Text = frm_lista_compras_dev_sr_agro.lote
                cmb_fecha_vencimiento.Value = frm_lista_compras_dev_sr_agro.f_vencimiento
                partidas()
                btn_cargar_compra.Enabled = False
            End If
        Catch ex As Exception
    ...
    Private Sub frm_nueva_devolucion_compra_nacional_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
    ...
    Private Sub btn_guardar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_guardar.Click
        ...
        Else
            ...
            Else
                ...
                dt_e.Columns.Add("F_VENCIMIENTO")
                dt_e.Columns.Add("CVE_COMPRA")
                dt_e.Columns.Add("FACT_SAE")
                ...
                dt_e.Rows.Add(txt_codigo_proveedor.Text, txt_nombre_proveedor.Text, cmb_sucursal.SelectedValue, txt_referencia_proveedor.Text, _
                              ...
                              Convert.ToDouble(txt_total_final.Text), Convert.ToDouble((txt_subtotalmostrador.Text)), _
                              Convert.ToDouble((txt_descuento_mostrador.Text)), Convert.ToDouble((txt_isvmostrador.Text)), _
                              Convert.ToDouble((txt_totalmostrador.Text)), txt_flete.Text, txt_lote.Text, cmb_fecha_vencimiento.Value, _
                              txt_cve_compra.Text, txtn_fact_sae.Text)
                ...
                dt_p.Columns.Add("COSTO")
                dt_p.Columns.Add("LOTE_ID")
                dt_p.Columns.Add("NUM_MOV")
                dt_p.Columns.Add("AJUSTE2")
                ...
                For index As Integer = 0 To grd_ingreso.Rows.Count - 1
                    dt_p.Rows.Add(grd_ingreso.Rows(index).Cells(0).Value, grd_ingreso.Rows(index).Cells(1).Value, _
                                    ...
                                    grd_ingreso.Rows(index).Cells(17).Value, grd_ingreso.Rows(index).Cells(18).Value, _
                                    grd_ingreso.Rows(index).Cells(19).Value, grd_ingreso.Rows(index).Cells(20).Value, _
                                    grd_ingreso.Rows(index).Cells(21).Value, grd_ingreso.Rows(index).Cells(22).Value, _
                                    grd_ingreso.Rows(index).Cells(23).Value, grd_ingreso.Rows(index).Cells(24).Value, _
                                    grd_ingreso.Rows(index).Cells(25).Value, grd_ingreso.Rows(index).Cells(26).Value, _
                ...
                If MessageBox.Show("¿Estan correctos los datos?", "Confirmacion", MessageBoxButtons.YesNoCancel, _
                    ...
                    Else
                        tipo = "P"
                    End If
                    resultado = conexion.guardar_devolucion_scompra(dt_e, dt_p, _Inicio.lblUsuario.Text, tipo)
    ...
    Sub calcular()
        ...
        For index As Integer = 0 To grd_ingreso.RowCount - 1
            subtotal_g += grd_ingreso.Rows(index).Cells(11).Value
            descuento_g += grd_ingreso.Rows(index).Cells(12).Value
            isv_g += grd_ingreso.Rows(index).Cells(13).Value
            total_g += grd_ingreso.Rows(index).Cells(14).Value

            subtotal_g_most += grd_ingreso.Rows(index).Cells(17).Value
            descuento_g_most += grd_ingreso.Rows(index).Cells(18).Value
            isv_g_most += grd_ingreso.Rows(index).Cells(19).Value
            total_g_most += grd_ingreso.Rows(index).Cells(20).Value
        Next
        ...
        txt_subtotalmostrador.Text = subtotal_g_most
        txt_descuento_mostrador.Text = descuento_g_most
        txt_isvmostrador.Text = isv_g_most
        txt_totalmostrador.Text = total_g_most
    End Sub
