Imports Disar.data
Public Class frm_nd_compras_cargill_sragro
    ...
    Private Sub btn_new_prov_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_new_prov.Click
        ...
        frm_lista_proveedores_sragro.cargill = "S"
    ...
    Sub cargar_producto_rutina()
        ...
        If frm_productos_precios_neg_sragro.ShowDialog = Windows.Forms.DialogResult.OK Then
            ...
            txt_peso.Text = frm_productos_precios_neg_sragro.peso
            txt_con_lote.Text = frm_productos_precios_neg_sragro.con_lote
            txt_descuento_por_PP.Text = Val(txt_descuento_pp.Text)
    ...
    Private Sub btn_agregar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_agregar.Click
        Try
            ...
            Else
                ...
                Dim descuento_por_articulo As Double = 0, descuento_por_articulo2 As Double = 0

                descuento_por_articulo += subtotal * (Val(txt_descuento_por_PP.Text) / 100)
                descuento_por_articulo2 += subtotal_mostrador * (Val(txt_descuento_por_PP.Text) / 100)

                MsgBox(Val(txt_descuento_por_PP.Text).ToString() + " " + descuento_por_articulo.ToString() + " " + descuento_por_articulo2.ToString())
                ...
                grd_ingreso.Rows.Add(Val(txt_cantidad.Text), txt_codigo_producto.Text, txt_nombre_producto.Text, txt_factor_conversion.Text, _
                                     ...
                                     Math.Round(descuento_mostrador), Math.Round(isv_mostrador), 0, Val(txt_peso.Text), txt_con_lote.Text, _
                                     0, 0, Val(txt_descuento_por_PP.Text), descuento_por_articulo, descuento_por_articulo2)
                limpiar()
                sumar_totales()
                txt_cantidad.Focus()
    ...
    Sub sumar_totales()
        ...
        Dim total_general1 As Double = 0, total_general2 As Double = 0

        Dim desc_pp1 As Double = 0, desc_pp2 As Double = 0
        ...
        For index As Integer = 0 To grd_ingreso.RowCount - 1
            ...
            isv += grd_ingreso.Rows(index).Cells(23).Value
            ...
            isv_mostrador += grd_ingreso.Rows(index).Cells(24).Value
            desc_mostrador += grd_ingreso.Rows(index).Cells(18).Value

            desc_pp1 += grd_ingreso.Rows(index).Cells(26).Value
            desc_pp2 += grd_ingreso.Rows(index).Cells(27).Value
        Next

        'total +=  Val(txt_flete.Text)
        'total_mostrador += Val(txt_flete.Text)

        'desc_pp1 = subtotal * (Val(txt_descuento_pp.Text) / 100)
        'desc_pp2 = subtotal_mostrador * (Val(txt_descuento_pp.Text) / 100)

        flete = Val(txt_flete.Text)

        total_general1 = total + Val(txt_flete.Text)
        total_general2 = total_mostrador + Val(txt_flete.Text) - desc_pp2 + Val(txt_isv_flete.Text)
        ...
        txt_flete2.Text = Val(txt_flete2.Text) + Val(txt_isv_flete.Text)

        txt_flete1.Text = flete.ToString("N2")
        txt_flete2.Text = (Val(txt_flete2.Text) + flete).ToString("N2")

        txt_tot_mostrador.Text = total_general1.ToString("N2")
        txt_tot_mostrador2.Text = total_general2.ToString("N2")

        txt_descuentopp1.Text = desc_pp1
        txt_descuentopp2.Text = desc_pp2
    End Sub

    Sub cargar_isv()
        Try
            For index As Integer = 0 To grd_ingreso.Rows.Count - 1
                Dim nuevo_isv As Double = 0, nuevo_isv2 As Double = 0
                nuevo_isv = (Convert.ToDouble(Val(grd_ingreso.Rows(index).Cells(9).Value)) - _
                             (Convert.ToDouble(Val(grd_ingreso.Rows(index).Cells(9).Value)) * Val(txt_descuento_pp.Text.Replace(",", "") / 100))) _
                             - Val(grd_ingreso.Rows(index).Cells(10).Value)

                nuevo_isv2 = (Convert.ToDouble(Val(grd_ingreso.Rows(index).Cells(16).Value)) - _
                              (Convert.ToDouble(Val(grd_ingreso.Rows(index).Cells(16).Value)) * Val(txt_descuento_pp.Text.Replace(",", "") / 100))) _
                              - Val(grd_ingreso.Rows(index).Cells(18).Value)

                grd_ingreso.Rows(index).Cells(23).Value = nuevo_isv * (grd_ingreso.Rows(index).Cells(8).Value / 100)
                grd_ingreso.Rows(index).Cells(24).Value = nuevo_isv2 * (grd_ingreso.Rows(index).Cells(8).Value / 100)
            Next
        Catch ex As Exception
            For index As Integer = 0 To grd_ingreso.Rows.Count - 1
                grd_ingreso.Rows(index).Cells(23).Value = grd_ingreso.Rows(index).Cells(11).Value
                grd_ingreso.Rows(index).Cells(24).Value = grd_ingreso.Rows(index).Cells(19).Value
            Next
        End Try
    End Sub
    ...
    Private Sub btn_guardar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_guardar.Click
        Try
            ...
            Else
                ...
                dt_e.Columns.Add("FLETE") '16
                dt_e.Columns.Add("NUMERO_FLETE") '17
                dt_e.Columns.Add("FECHA_FACTURA") '18

                dt_e.Columns.Add("ISV_FLETE")
                dt_e.Columns.Add("TOT_FLETE")
                ...
                dt_e.Rows.Add(txt_codigo_proveedor.Text, txt_nombre_proveedor.Text, cmb_sucursal.SelectedValue, txt_referencia_proveedor.Text, _
                              txt_rfc.Text, cmb_fecha_documento.Value.Date, txt_descuento_general.Text, _
                            Convert.ToDouble(txt_tot_mostrador.Text) - Convert.ToDouble(txt_isv_final.Text.Replace(",", "")), _
                              Convert.ToDouble(txt_descuento_final.Text), Convert.ToDouble(txt_isv_final.Text.Replace(",", "")), _
                              Convert.ToDouble(txt_tot_mostrador.Text), _
                              "PENDIENTE", Convert.ToDouble((txt_subtotalmostrador.Text)), Convert.ToDouble((txt_descuento_mostrador.Text)) _
                              , Convert.ToDouble((txt_isvmostrador.Text)), Convert.ToDouble((txt_totalmostrador.Text)), _
                              Convert.ToDouble(Val(txt_flete.Text.Replace(",", ""))), txtNumFlete.Text, cmb_fecha_factura.Value, _
                              Convert.ToDouble(Val(txt_isv_flete.Text)), Convert.ToDouble(Val(txt_total_flete.Text)))
                ...
                dt_p.Columns.Add("PESO")
                dt_p.Columns.Add("CON_LOTE")

                dt_p.Columns.Add("PORC_PP")
                dt_p.Columns.Add("DESCUENTO_PP")
                dt_p.Columns.Add("DESCUENTO_PP_MOSTR")
                ...
                For index As Integer = 0 To grd_ingreso.Rows.Count - 1
                    dt_p.Rows.Add(grd_ingreso.Rows(index).Cells(4).Value, grd_ingreso.Rows(index).Cells(1).Value, grd_ingreso.Rows(index).Cells(2).Value, _
                                  ...
                                  , grd_ingreso.Rows(index).Cells(19).Value, grd_ingreso.Rows(index).Cells(17).Value, grd_ingreso.Rows(index).Cells(20).Value, _
                                  grd_ingreso.Rows(index).Cells(21).Value, grd_ingreso.Rows(index).Cells(22).Value, grd_ingreso.Rows(index).Cells(25).Value, grd_ingreso.Rows(index).Cells(26).Value, grd_ingreso.Rows(index).Cells(27).Value)
                Next
                ...
                If modo = "nuevo" Then
                    ...
                    If MessageBox.Show("¿Desea enviar la compra a SAE?", "Confirmacion", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                        resultado = conexion.guardar_compras_local_envio_SAE_CARGILL(dt_e, dt_p, conlote, lote, _Inicio.lblUsuario.Text)
                    Else
                        resultado = conexion.guardar_compras_local_CARGILL(dt_e, dt_p)
                    End If

                ElseIf modo = "modificar" Then
                    resultado = conexion.modificar_compras_CARGILL(dt_e, dt_p, codigo_compra)
                    If resultado = "correcto" Then
                        If MessageBox.Show("¿Desea enviar la compra a SAE?", "Confirmacion", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                            resultado = conexion.envio_individual_compras_SAE_CARGILL(dt_e, dt_p, codigo_compra, conlote, lote, _Inicio.lblUsuario.Text)
                        End If
                    End If
                End If
        ...
        Try
            ...
            For index As Integer = 0 To dt.Rows.Count - 1
                grd_ingreso.Rows.Add(dt.Rows(index)(4), dt.Rows(index)(1), dt.Rows(index)(2), dt.Rows(index)(3), _
                                     ...
                                     dt.Rows(index)(18), dt.Rows(index)(16), dt.Rows(index)(17), dt.Rows(index)(19), dt.Rows(index)(20), dt.Rows(index)(21), dt.Rows(index)(22), dt.Rows(index)(23), dt.Rows(index)(24))
            Next
    ...
    Private Sub cmb_fecha_documento_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmb_fecha_documento.ValueChanged, cmb_fecha_factura.ValueChanged
    ...
    Private Sub frm_nd_compras_cargill_sragro_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Sub calcular_nuevo_costo_final()
        Try
            ...
            For index As Integer = 0 To grd_ingreso.RowCount - 1
                peso_total += (grd_ingreso.Rows(index).Cells(21).Value * grd_ingreso.Rows(index).Cells(0).Value) '* grd_ingreso.Rows(index).Cells(3).Value)
            Next
            For index As Integer = 0 To grd_ingreso.RowCount - 1
                Dim precio_neto As Double = 0
                Dim flete As Double = 0

                If peso_total > 0 Then
                    'flete por libra
                    flete = Convert.ToDouble(Val(txt_flete.Text)) / peso_total
                    flete = ((grd_ingreso.Rows(index).Cells(21).Value)) * flete
                    'flete = Convert.ToDouble(Val(txt_flete.Text)) / (unidades * (grd_ingreso.Rows(index).Cells(21).Value))
                Else
                    flete = 0
                End If

                'flete = ((grd_ingreso.Rows(index).Cells(21).Value * grd_ingreso.Rows(index).Cells(0).Value * grd_ingreso.Rows(index).Cells(3).Value)) / peso_total * Convert.ToDouble(Val(txt_flete.Text))


                'flete = flete * grd_ingreso.Rows(index).Cells(21).Value
                precio_neto = grd_ingreso.Rows(index).Cells(5).Value + flete
                grd_ingreso.Rows(index).Cells(20).Value = precio_neto
            Next
        Catch ex As Exception
    ...
    Private Sub txt_flete_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If modo = "vista" Then
            txt_flete1.Text = Val(txt_flete.Text).ToString("N2")
            txt_flete2.Text = Val(txt_flete.Text).ToString("N2")
        Else
            calcular_nuevo_costo_final()
            sumar_totales()
        End If
    ...
    Private Sub txt_descuento_pp_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txt_descuento_pp.TextChanged
        calcular_nuevo_costo_final()
        sumar_totales()
    End Sub

    Private Sub txt_flete_TextChanged_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txt_flete.TextChanged
        txt_total_flete.Text = Val(txt_flete.Text) + Val(txt_isv_flete.Text)
    End Sub

    Private Sub txt_isv_flete_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txt_isv_flete.TextChanged
        txt_total_flete.Text = Val(txt_flete.Text) + Val(txt_isv_flete.Text)
    End Sub
End Class