Imports Disar.data
Public Class frm_nueva_devolucion_SrAgro
    ...
    Private Sub frm_nuevo_descuento_tactico_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        If modo = "visualice" Then
            ...
            Try
                grd_productos.DataSource = conexion.Detalle_Devoluciones(txt_num_registro.Text)
    ...
    Sub cargar_factura()
        Try
            Frm_FacturasSrAgro.Close()
            Frm_FacturasSrAgro.con_credito = "devolucion"
            Frm_FacturasSrAgro.ShowDialog()
            Frm_FacturasSrAgro.txt_numero_factura.Focus()
            If Frm_FacturasSrAgro.DialogResult = DialogResult.OK Then
                txt_numero_factura.Text = Frm_FacturasSrAgro.factura
                txt_tipo.Text = Frm_FacturasSrAgro.tipo
                cmb_fecha_documento.Value = Frm_FacturasSrAgro.fecha
                cmb_fecha_entrega.Value = Frm_FacturasSrAgro.fecha_entrega
                ...
                cmb_sucursal.SelectedValue = Frm_FacturasSrAgro.almacen
                txt_codigo_entregador.Text = Frm_FacturasSrAgro.cod_entregador
                txt_entregador.Text = Frm_FacturasSrAgro.entregador
                txt_vehiculo.Text = Frm_FacturasSrAgro.vehiculo
                txt_codigo_cliente.Text = Frm_FacturasSrAgro.cod_cliente
                txt_nombre_cliente.Text = Frm_FacturasSrAgro.cliente
                txt_codigo_vendedor.Text = Frm_FacturasSrAgro.cod_vendedor
                txt_nombre_vendedor.Text = Frm_FacturasSrAgro.vendedor
                cmb_fecha_vencimiento.Value = Frm_FacturasSrAgro.fecha_ven
                txt_condicion.Text = Frm_FacturasSrAgro.condicion
                txt_rfc.Text = Frm_FacturasSrAgro.rfc
                txt_autoriza.Text = Frm_FacturasSrAgro.autoriza
                txt_folio.Text = Frm_FacturasSrAgro.folio
                txt_autoanio.Text = Frm_FacturasSrAgro.autoanio
            End If
        Catch ex As Exception
            MessageBox.Show(ex.Message)
        End Try
    ...
    Private Sub btn_productos_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        frm_productos_dt_SrAgro2.Close()
        frm_productos_dt_SrAgro2.ShowDialog()
        If frm_productos_dt_SrAgro2.DialogResult = Windows.Forms.DialogResult.OK Then
            txt_codigo_producto.Text = frm_productos_dt_SrAgro2.cve_art
            txt_nombre_producto.Text = frm_productos_dt_SrAgro2.producto
        End If
    End Sub
                ...
                Else
                    ...
                    If MessageBox.Show("¿Estan correctos los datos?", "Confirmacion", MessageBoxButtons.YesNo, _
                        ...
                        ElseIf modo = "ins" Then
                            variable_error = conexion.guardar_datos_local(dt_encabezado, dt_partidas)
                        End If

                        If variable_error = "correcto" Then
                            Dim conexion_bita As New cls_bitacora_reporteador
                            conexion_bita.RegistraBitacora(Now, _Inicio.lblUsuario.Text, "Ingreso Nueva Devolucion", "Almacen - Devoluciones", _
