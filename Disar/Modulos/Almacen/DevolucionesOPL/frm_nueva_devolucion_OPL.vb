Imports Disar.data
Public Class frm_nueva_devolucion_OPL
    ...
    Private Sub frm_nuevo_descuento_tactico_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        If modo = "visualice" Then
            ...
            Try
                grd_productos.DataSource = conexion.Detalle_DevolucionesOPL(txt_num_registro.Text)
    ...
    Sub cargar_conceptos()
        Try
            cmb_concepto.DataSource = conexion.listar_conceptosOPL
    ...
    Sub cargar_factura()
        Try
            ...
            frm_facturas_OPL.con_credito = "devolucion"
            frm_facturas_OPL.ShowDialog()
            frm_facturas_OPL.txt_numero_factura.Focus()
            If frm_facturas_OPL.DialogResult = DialogResult.OK Then
                txt_numero_factura.Text = frm_facturas_OPL.factura
                txt_tipo.Text = frm_facturas_OPL.tipo
                cmb_fecha_documento.Value = frm_facturas_OPL.fecha
                cmb_fecha_entrega.Value = frm_facturas_OPL.fecha_entrega
                ...
                cmb_sucursal.SelectedValue = frm_facturas_OPL.almacen
                txt_codigo_entregador.Text = frm_facturas_OPL.cod_entregador
                txt_entregador.Text = frm_facturas_OPL.entregador
                txt_vehiculo.Text = frm_facturas_OPL.vehiculo
                txt_codigo_cliente.Text = frm_facturas_OPL.cod_cliente
                txt_nombre_cliente.Text = frm_facturas_OPL.cliente
                txt_codigo_vendedor.Text = frm_facturas_OPL.cod_vendedor
                txt_nombre_vendedor.Text = frm_facturas_OPL.vendedor
                cmb_fecha_vencimiento.Value = frm_facturas_OPL.fecha_ven
                txt_condicion.Text = frm_facturas_OPL.condicion
                txt_rfc.Text = frm_facturas_OPL.rfc
                txt_autoriza.Text = frm_facturas_OPL.autoriza
                txt_folio.Text = frm_facturas_OPL.folio
                txt_autoanio.Text = frm_facturas_OPL.autoanio
            End If
        Catch ex As Exception
            MessageBox.Show(ex.Message)
        End Try
                ...
                Else
                    ...
                    If MessageBox.Show("¿Estan correctos los datos?", "Confirmacion", MessageBoxButtons.YesNo, _
                        ...
                        ElseIf modo = "ins" Then
                            variable_error = conexion.guardar_datos_local_OPL(dt_encabezado, dt_partidas)
                        End If

                        If variable_error = "correcto" Then
                            Dim conexion_bita As New cls_bitacora_reporteador
                            conexion_bita.RegistraBitacora(Now, _Inicio.lblUsuario.Text, "Ingreso Nueva Devolucion OPL", "Almacen - Devoluciones - OPL",
