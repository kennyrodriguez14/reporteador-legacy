...
Public Class cls_efectividad_src
    ...
    Public Function Efectivos(ByVal Fecha As Date, ByVal Clave As Integer) As DataView
        ...
        Dim sSQL As String = "SELECT  DISTINCT FACTF05.CVE_CLPV, FACTF05.FECHA_DOC " & _
                             "FROM dbo.FACTF05 " & _
                             "WHERE (FECHA_DOC = @Fecha) and (CVE_VEND=@Clave) and NUM_ALMA =2"
        ...
        Catch ex As SqlException
            MsgBox("Hay un problema con la conexión a los datos 2A" & ex.Message, MsgBoxStyle.Critical)
            ActualizarConexion.Close()
        End Try
        Return dv
    End Function
    ...
    Public Function EfectivosMenores30(ByVal Fecha As Date, ByVal Clave As Integer) As DataView
        ...
        Dim sSQL As String = "SELECT  DISTINCT FACTF05.CVE_CLPV, FACTF05.FECHA_DOC " & _
                             "FROM dbo.FACTF05 " & _
                             "WHERE (FECHA_DOC = @Fecha) and (CVE_VEND=@Clave) and NUM_ALMA =2  AND (IMPORTE < 30000)"
        Dim miComando As New SqlCommand(sSQL, ActualizarConexion)
        miComando.Parameters.AddWithValue("@Fecha", Fecha)
        miComando.Parameters.AddWithValue("@Clave", Clave)
        ActualizarConexion.Open()
    ...
    Public Function Visitados(ByVal Vendedor As Integer, ByVal Dia As String) As DataView
        Dim ActualizarConexion As New SqlConnection(Conexion.CadenaSQL())
        Dim sSQL As String = "SELECT count(dbo.CLIE05.CLAVE) FROM dbo.CLIE05 INNER JOIN dbo.VEND05 ON  " & _
                             "dbo.CLIE05.CVE_VEND = dbo.VEND05.CVE_VEND LEFT OUTER JOIN dbo.CLIE_CLIB05 ON  " & _
                             "dbo.CLIE05.CLAVE = dbo.CLIE_CLIB05.CVE_CLIE WHERE (dbo.CLIE05.CVE_VEND =@Vendedor) AND " & _
                             "(dbo.CLIE_CLIB05.CAMPLIB3 = @Dia OR dbo.CLIE_CLIB05.CAMPLIB3 = SUBSTRING(@Dia, 1, LEN(@Dia) - 2)) and  (dbo.CLIE05.STATUS <> 'S') "
        Dim miComando As New SqlCommand(sSQL, ActualizarConexion)
        ...
        Try
            ...
            ActualizarConexion.Close()
        Catch ex As SqlException
            MsgBox("Hay un problema con la conexión a los datos 2 Saburba " & ex.Message, MsgBoxStyle.Critical)
            ActualizarConexion.Close()
        End Try
        Return dv
    End Function
    ...
    Public Function EfectivosRangoFechas(ByVal Fecha As Date, ByVal Fecha2 As Date, ByVal Clave As Integer) As DataView
        ...
        Dim sSQL As String = "SELECT DISTINCT TOP (100) PERCENT CVE_CLPV, FECHA_DOC " & _
                             "FROM dbo.FACTF05 " & _
                             "WHERE (FECHA_DOC >= @Fecha) AND (CVE_VEND=@Clave) AND (FECHA_DOC <= @Fecha2) and NUM_ALMA =2"
        Dim miComando As New SqlCommand(sSQL, ActualizarConexion)
        miComando.Parameters.AddWithValue("@Fecha", Fecha)
    ...
    Public Function EfectivosRangoFechasMenores30(ByVal Fecha As Date, ByVal Fecha2 As Date, ByVal Clave As Integer) As DataView
        ...
        Dim sSQL As String = "SELECT DISTINCT TOP (100) PERCENT CVE_CLPV, FECHA_DOC " & _
                             "FROM dbo.FACTF05 " & _
                             "WHERE (FECHA_DOC >= @Fecha) AND (CVE_VEND=@Clave) AND (FECHA_DOC <= @Fecha2) and NUM_ALMA =2 AND (IMPORTE < 30000)"
        Dim miComando As New SqlCommand(sSQL, ActualizarConexion)
        miComando.Parameters.AddWithValue("@Fecha", Fecha)
        miComando.Parameters.AddWithValue("@Fecha2", Fecha2)
        miComando.Parameters.AddWithValue("@Clave", Clave)
    ...
    Public Function VisitadosRangoFechas(ByVal Vendedor As Integer, ByVal Dia As String, ByVal Dia2 As String, ByVal Dia3 As String) As DataView
        Dim ActualizarConexion As New SqlConnection(Conexion.CadenaSQL())
        Dim sSQL As String = "SELECT count(dbo.CLIE05.CLAVE) FROM dbo.CLIE05 INNER JOIN dbo.VEND05 ON " & _
            "dbo.CLIE05.CVE_VEND = dbo.VEND05.CVE_VEND LEFT OUTER JOIN dbo.CLIE_CLIB05 ON dbo.CLIE05.CLAVE =  " & _
            "dbo.CLIE_CLIB05.CVE_CLIE WHERE (dbo.CLIE05.CVE_VEND =@Vendedor) and " & _
            "((dbo.CLIE_CLIB05.CAMPLIB3 = @Dia) or (dbo.CLIE_CLIB05.CAMPLIB3 = @Dia2) or (dbo.CLIE_CLIB05.CAMPLIB3 = @Dia3)) " & _
            " and  (dbo.CLIE05.STATUS <> 'S') "
        Dim miComando As New SqlCommand(sSQL, ActualizarConexion)
        miComando.Parameters.AddWithValue("@Vendedor", Vendedor)
        miComando.Parameters.AddWithValue("@Dia", Dia)
    ...
    Public Function NotasCDiarias(ByVal Fecha As Date, ByVal Clave As Integer)
        ...
        Dim sSQL As String = "Select DISTINCT(dbo.FACTD05.CVE_CLPV) " & _
                            ...
                            "(dbo.FACTD05.SERIE = 'N') AND (dbo.FACTD05.STATUS <>'C') AND (dbo.FACTD05.FECHA_ENT < dbo.FACTD05.FECHA_DOC) and NUM_ALMA=2"
        Dim miComando As New SqlCommand(sSQL, ActualizarConexion)
    ...
    Public Function NotasCredito(ByVal Fecha As Date, ByVal Fecha2 As Date, ByVal Clave As Integer) As DataView
        ...
        "dbo.CLIE05 ON dbo.FACTD05.CVE_CLPV = dbo.CLIE05.CLAVE " & _
        " WHERE (dbo.FACTD05.FECHA_DOC >= @Fecha) AND (dbo.FACTD05.CVE_VEND = @Clave) AND " & _
        "(dbo.FACTD05.FECHA_DOC <= @Fecha2) AND (dbo.FACTD05.SERIE = 'N') AND (dbo.FACTD05.STATUS <>'C') AND (dbo.FACTD05.FECHA_ENT < dbo.FACTD05.FECHA_DOC) and NUM_ALMA=2"
    ...
    Public Function NotasCreditoGeneral(ByVal Fecha As Date, ByVal Fecha2 As Date, ByVal Clave As Integer) As DataView
        ...
        "dbo.CLIE05 ON dbo.FACTD05.CVE_CLPV = dbo.CLIE05.CLAVE " & _
        " WHERE (dbo.FACTD05.FECHA_DOC >= @Fecha) AND (dbo.FACTD05.CVE_VEND = @Clave) AND " & _
        "(dbo.FACTD05.FECHA_DOC <= @Fecha2) AND (dbo.FACTD05.SERIE = 'N') AND (dbo.FACTD05.STATUS <>'C') AND (dbo.FACTD05.FECHA_ENT < dbo.FACTD05.FECHA_DOC)"
    ...
    Public Function VisitadosDIMOSA(ByVal Vendedor As Integer, ByVal Dia As String) As DataView
        Dim ActualizarConexion As New SqlConnection(ConexionDIMOSA.CadenaSQL())
        Dim sSQL As String = "SELECT count(dbo.CLIE06.CLAVE) FROM dbo.CLIE06 INNER JOIN dbo.VEND06 ON  " & _
                             "dbo.CLIE06.CVE_VEND = dbo.VEND06.CVE_VEND LEFT OUTER JOIN dbo.CLIE_CLIB06 ON  " & _
                             "dbo.CLIE06.CLAVE = dbo.CLIE_CLIB06.CVE_CLIE WHERE (dbo.CLIE06.CVE_VEND =@Vendedor) AND " & _
                             "(dbo.CLIE_CLIB06.CAMPLIB3 = @Dia OR dbo.CLIE_CLIB06.CAMPLIB3 = SUBSTRING(@Dia, 1, LEN(@Dia) - 2)) and  (dbo.CLIE06.STATUS <> 'S') "
        Dim miComando As New SqlCommand(sSQL, ActualizarConexion)
        miComando.Parameters.AddWithValue("@Vendedor", Vendedor)
        miComando.Parameters.AddWithValue("@Dia", Dia)
        ActualizarConexion.Open()
    ...
    Public Function VisitadosRangoFechasDIMOSA(ByVal Vendedor As Integer, ByVal Dia As String, ByVal Dia2 As String, ByVal Dia3 As String) As DataView
        Dim ActualizarConexion As New SqlConnection(ConexionDIMOSA.CadenaSQL())
        Dim sSQL As String = "SELECT count(dbo.CLIE06.CLAVE) FROM dbo.CLIE06 INNER JOIN dbo.VEND06 ON " & _
            "dbo.CLIE06.CVE_VEND = dbo.VEND06.CVE_VEND LEFT OUTER JOIN dbo.CLIE_CLIB06 ON dbo.CLIE06.CLAVE =  " & _
            "dbo.CLIE_CLIB06.CVE_CLIE WHERE (dbo.CLIE06.CVE_VEND =@Vendedor) and " & _
            "((dbo.CLIE_CLIB06.CAMPLIB3 = @Dia) or (dbo.CLIE_CLIB06.CAMPLIB3 = @Dia2) or (dbo.CLIE_CLIB06.CAMPLIB3 = @Dia3)) " & _
            " and  (dbo.CLIE06.STATUS <> 'S') "
        Dim miComando As New SqlCommand(sSQL, ActualizarConexion)
        miComando.Parameters.AddWithValue("@Vendedor", Vendedor)
        miComando.Parameters.AddWithValue("@Dia", Dia)
        miComando.Parameters.AddWithValue("@Dia2", Dia2)
    ...
    Public Function NotasCDiariasDIMOSA(ByVal Fecha As Date, ByVal Clave As Integer)
        ...
        Dim sSQL As String = "Select DISTINCT(dbo.FACTD06.CVE_CLPV) " & _
                            ...
                            "(dbo.FACTD06.SERIE = 'N') AND (dbo.FACTD06.STATUS <>'C') AND (dbo.FACTD06.FECHA_ENT < dbo.FACTD06.FECHA_DOC) and NUM_ALMA=2"
        Dim miComando As New SqlCommand(sSQL, ActualizarConexion)
        miComando.Parameters.AddWithValue("@Fecha", Fecha.Date)
        miComando.Parameters.AddWithValue("@Clave", Clave)
        ActualizarConexion.Open()
    ...
    Public Function NotasCreditoDIMOSA(ByVal Fecha As Date, ByVal Fecha2 As Date, ByVal Clave As Integer) As DataView
        ...
        "dbo.CLIE06 ON dbo.FACTD06.CVE_CLPV = dbo.CLIE06.CLAVE " & _
        " WHERE (dbo.FACTD06.FECHA_DOC >= @Fecha) AND (dbo.FACTD06.CVE_VEND = @Clave) AND " & _
        "(dbo.FACTD06.FECHA_DOC <= @Fecha2) AND (dbo.FACTD06.SERIE = 'N') AND (dbo.FACTD06.STATUS <>'C') AND (dbo.FACTD06.FECHA_ENT < dbo.FACTD06.FECHA_DOC) and NUM_ALMA=2"
    ...
    Public Function NotasCreditoGeneralDIMOSA(ByVal Fecha As Date, ByVal Fecha2 As Date, ByVal Clave As Integer) As DataView
        Dim ActualizarConexion As New SqlConnection(ConexionDIMOSA.CadenaSQL())
        Dim sSQL As String = "Select DISTINCT(dbo.FACTD06.CVE_CLPV) " & _
        "FROM  dbo.PAR_FACTD06 LEFT OUTER JOIN " & _
        "dbo.INVE06 ON dbo.PAR_FACTD06.CVE_ART = dbo.INVE06.CVE_ART INNER JOIN " & _
        ...
        "dbo.CLIE06 ON dbo.FACTD06.CVE_CLPV = dbo.CLIE06.CLAVE " & _
        " WHERE (dbo.FACTD06.FECHA_DOC >= @Fecha) AND (dbo.FACTD06.CVE_VEND = @Clave) AND " & _
        "(dbo.FACTD06.FECHA_DOC <= @Fecha2) AND (dbo.FACTD06.SERIE = 'N') AND (dbo.FACTD06.STATUS <>'C') AND (dbo.FACTD06.FECHA_ENT < dbo.FACTD06.FECHA_DOC)"
    ...
    Public Function EfectivosGeneralDIMOSA(ByVal Fecha As Date, ByVal Clave As Integer) As DataView
        Dim ActualizarConexion As New SqlConnection(ConexionDIMOSA.CadenaSQL())
        Dim sSQL As String = "SELECT  DISTINCT FACTF06.CVE_CLPV, FACTF06.FECHA_DOC " & _
                             "FROM dbo.FACTF06 " & _
                             "WHERE (FECHA_DOC = @Fecha) and (CVE_VEND=@Clave)"
    ...
    Public Function EfectivosRangoFechasGeneralDIMOSA(ByVal Fecha As Date, ByVal Fecha2 As Date, ByVal Clave As Integer) As DataView
        Dim ActualizarConexion As New SqlConnection(ConexionDIMOSA.CadenaSQL())
        Dim sSQL As String = "SELECT DISTINCT TOP (100) PERCENT CVE_CLPV, FECHA_DOC " & _
                             "FROM dbo.FACTF06 " & _
                             "WHERE (FECHA_DOC >= @Fecha) AND (CVE_VEND=@Clave) AND (FECHA_DOC <= @Fecha2) "
