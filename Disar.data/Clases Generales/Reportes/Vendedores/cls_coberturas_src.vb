...
Public Class cls_coberturas_src
    ...
    Public Function ListaUniverso() As DataView
        ...
        " CLIE05.CVE_VEND =  dbo.VEND05.CVE_VEND) AND (dbo.VEND05.CORREOE = 'SR@C')  " & _
    ...
    Public Function VentasXLinea(ByVal Fecha_Ini As Date, ByVal Fecha_Fin As Date, ByVal Linea As String, ByVal Clave As Integer) As DataView
        Dim ActualizarConexion As New SqlConnection(Conexion.CadenaSQL())
        ...
        " (dbo.FACTF05.FECHA_DOC <= @Fin) AND (dbo.FACTF05.NUM_ALMA = 2)  " & _
        ...
        " (dbo.FACTD05.FECHA_DOC <= @Fin) AND (dbo.FACTD05.NUM_ALMA = 2) " & _
    ...
    Public Function VentasXLineaKC(ByVal Fecha_Ini As Date, ByVal Fecha_Fin As Date, ByVal Clave As Integer) As DataView
        Dim ActualizarConexion As New SqlConnection(Conexion.CadenaSQL())
        Dim sSQL As String = "SELECT        CVE_CLPV, sum(Expr1) as Suma " & _
        " FROM " & _
        "( " & _
    ...
    Public Function VentasXLineaOTROS(ByVal Fecha_Ini As Date, ByVal Fecha_Fin As Date, ByVal Clave As Integer) As DataView
        Dim ActualizarConexion As New SqlConnection(Conexion.CadenaSQL())
        Dim sSQL As String = "SELECT        CVE_CLPV, sum(Expr1) as Suma " & _
        " FROM " & _
        "( " & _
    ...
    Public Function VentasXProd(ByVal Fecha_Ini As Date, ByVal Fecha_Fin As Date, ByVal Prod As String, ByVal Clave As Integer, ByVal Almacen As Integer) As DataView
        Dim ActualizarConexion As New SqlConnection(Conexion.CadenaSQL())
        Dim sSQL As String = "SELECT        CVE_CLPV, sum(Expr1) as Suma " & _
        " FROM " & _
        "( " & _
        ...
        " dbo.VEND05 ON dbo.CLIE05.CVE_VEND = dbo.VEND05.CVE_VEND ON dbo.FACTF05.CVE_CLPV = dbo.CLIE05.CLAVE INNER JOIN " & _
        " dbo.INVE05 ON dbo.PAR_FACTF05.CVE_ART = dbo.INVE05.CVE_ART  " & _
        " WHERE        (ISNUMERIC(dbo.VEND05.CVE_VEND) = 1) AND (dbo.VEND05.CVE_VEND = @Clave) AND (dbo.INVE05.CVE_ART = @Linea) AND (dbo.FACTF05.FECHA_DOC >= @Inicio) AND  " & _
        " (dbo.FACTF05.FECHA_DOC <= @Fin) AND (dbo.FACTF05.NUM_ALMA = @Almacen)  " & _
        " GROUP BY dbo.FACTF05.CVE_CLPV  " & _
        " UNION ALL " & _
        " SELECT        dbo.FACTD05.CVE_CLPV, SUM(dbo.PAR_FACTD05.PREC * dbo.PAR_FACTD05.CANT * - 1) AS Expr1 " & _
        " FROM            dbo.PAR_FACTD05 INNER JOIN  " & _
        ...
        " dbo.VEND05 AS VEND05_1 ON CLIE05_1.CVE_VEND = VEND05_1.CVE_VEND ON dbo.FACTD05.CVE_CLPV = CLIE05_1.CLAVE INNER JOIN " & _
        " dbo.INVE05 AS INVE05_1 ON dbo.PAR_FACTD05.CVE_ART = INVE05_1.CVE_ART " & _
        " WHERE        (ISNUMERIC(VEND05_1.CVE_VEND) = 1) AND (VEND05_1.CVE_VEND = @Clave) AND (INVE05_1.CVE_ART = @Linea) AND (dbo.FACTD05.FECHA_DOC >= @Inicio) AND " & _
        " (dbo.FACTD05.FECHA_DOC <= @Fin) AND (dbo.FACTD05.NUM_ALMA = @Almacen) " & _
        " GROUP BY dbo.FACTD05.CVE_CLPV) AS derivedtbl_1 " & _
        " Group By CVE_CLPV 		 Having sum(Expr1) > 0"
        ...
        miComando.Parameters.AddWithValue("@Inicio", Fecha_Ini)
        miComando.Parameters.AddWithValue("@Fin", Fecha_Fin)
        miComando.Parameters.AddWithValue("@Linea", Prod)
        miComando.Parameters.AddWithValue("@Clave", Clave)
